...
const PanelTourCard = ({ tour,onAccept,onReject}) => {
    ...
    const acceptTour = (id) => {
      ...
      if (access_token) {
        ...
        apiInstance
          .put(`/tours/${id}/admin_acceptance/`, {}, { headers })
          .then((res) => {
            toast.success('تور به لیست تورهای تایید شده اضافه شد',{duration: 3000   })
            onAccept(id)
          })
          .catch((err) => {
            console.log(err);
    ...
    const rejectTour = (id) => {
      ...
      if (access_token) {
        ...
        apiInstance
          .put(`/tours/${id}/admin_denial/`, {}, { headers })
          .then((res) => {
            toast.error('تور حذف شد', {duration: 3000 })
            onReject(id); 
          })
          .catch((err) => {
            console.log(err);
